...
- **Evaluation from day one:** a golden set of ~12 mechanically-determined queries with known-good expected cards, plus a hit-rate script. Recall is measured on the retrieved candidate pool, not on the curated recommendation — curation is meant to vary. It certifies mechanical retrieval and nothing else: curation is deliberately unmeasured, and flavor-retrieval quality is under-sampled by the golden set, so both are left to human judgment. recall@k serves two purposes — comparing candidate embedding configurations, and guarding a fixed one against drift. Changing the embedding model, channel set, or dimension starts a new baseline rather than continuing the old one. See [ADR 0006](adr/0006-eval-measures-retrieval-recall.md) and [ADR 0011](adr/0011-evaluation-scope-and-baseline-semantics.md).
...
| **Retrieval** | metadata filters (format legality, color identity) + per-channel semantic search + RRF fusion into one candidate pool | to build |
...
| **Evals** | golden query set + hit-rate script | to build |
