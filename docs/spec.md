...
- **Evaluation from day one:** a golden set of ~12 mechanically-determined queries with known-good expected cards, plus a hit-rate script. Recall is measured on the retrieved candidate pool, not on the curated recommendation — curation is meant to vary. This certifies mechanical retrieval only: not curation (deliberately unmeasured) and not flavor-retrieval quality (the golden set under-samples it), both of which are left to human judgment. recall@k is used two ways — comparing candidate embedding configurations head-to-head, and guarding a fixed configuration against drift — and a change to the embedding model, channel set, or dimension starts a new baseline rather than continuing the old one. See [ADR 0006](adr/0006-eval-measures-retrieval-recall.md) and [ADR 0011](adr/0011-evaluation-scope-and-baseline-semantics.md).
