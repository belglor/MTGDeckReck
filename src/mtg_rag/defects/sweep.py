...
def run_plan_sweep(
    *,
    ...
    """Plan each theme and score the queries, one `RunScores` per theme.
    ...
    A theme whose plan never validates gets a row with `plan=None` and the
    failure in `error`, and the sweep continues to the next one.
    """
